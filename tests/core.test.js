...
import { calculateMetrics, calculateNormalized, calculateDecision, normalize, normalizeResults, podium, percentage, METRICS } from '../src/lib/core.js';
...
test('podium uses column-normalized results, dense ties, and includes originally negative options', () => {
  const ranks = podium(normalizeResults([-0.004, 0.51231, 0.51234, 0.4, 0, 1.004].map((result, id) => ({ id, result }))));
  ...
  const negative = podium(normalizeResults([{ result: -2, id: 0 }, { result: -1, id: 1 }]));
  ...
  assert.deepEqual(podium(normalizeResults([])).map(r => r.options), [[], [], []]);
});
test('every result column independently maps minimum/maximum to 0/1 without changing source metrics', () => {
  ...
  assert.deepEqual(display.map(r => r.result), [0, 0.25, 1]);
  assert.deepEqual(display.map(r => r.balance), [0, 1, 0.5]);
  assert.deepEqual(display.map(r => r.fear), [null, 0, 1]);
  assert.deepEqual(display.map(r => r.calm), [0.5, 0.5, 0.5]);
  assert.deepEqual(display.map(r => r.uncertainty), [null, null, null]);
  assert.deepEqual(raw, before);
  assert.equal(display[0].label, 'A');
  const single = normalizeResults([{ result: 25, calm: null }])[0];
  assert.equal(single.result, 0.5); assert.equal(single.calm, null);
});
