...
const request = {
  description: 'I am deciding whether to move to another city.',
  intent: 'initial',
  options: [],
  pairs: [],
  messages: [],
...
/** Start an ephemeral API server for one test and close it during cleanup. */
...
/** Send a JSON suggestion request to a test server. */
const post = (base, payload, headers = {}) =>
...
test('demo API works without a key and honors language and capacity', async (t) => {
  ...
  assert.equal(data.pairs.length, 8);
  const ru = demoSuggestions({ ...request, description: 'Я хочу переехать в другой город, но не знаю куда.' });
  assert.match(ru.message, /примеры/);
  assert.match(ru.pairs[0].negativeLabel, /напряжение/);
...
test('OpenAI contract uses structured Responses output without storing the response', async () => {
  ...
  assert.deepEqual(
    await openAISuggestions(request, { apiKey: 'test-only', model: 'configured-model', fetchImpl }),
    output,
  );
