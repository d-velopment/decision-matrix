...
import { createState, addOption, addPair, loadState, saveState, addMentionedOptions, removeOption } from '../src/lib/state.js';
...
test('center produces equal ratings for 2–6 options regardless of angular arrangement', () => {
  for (let n = 2; n <= 6; n++) {
    const state = decision(n); const pair = state.pairs[0];
...
test('distance increases preference, clamp stays in the circle, and zero floor remains separate', () => {
  ...
  const point = clampPoint(3, 4); assert.equal(Math.hypot(point.x, point.y), 1);
  control.y = -0.3;
  const ratings = radialRatings(control, options); assert.ok(ratings.a > ratings.b);
});
test('negative/positive positions are independent, and rotating an option changes its distance', () => {
  const state = decision(2); const pair = state.pairs[0];
...
test('positions and ratings survive reload; old slider states migrate without losing values', () => {
  const state = decision(); const pair = state.pairs[0];
  moveControl(pair, state.options, 'positive', 0.4, -0.2);
  moveOption(pair, state.options, 'positive', state.options[0].id, 0.6);
  let saved; const storage = { setItem: (_, data) => { saved = data; }, getItem: () => saved };
  saveState(storage, state); assert.deepEqual(loadState(storage), state);
...
test('auto-added mentions honor duplicates, deletion, and the option cap', () => {
  ...
  addMentionedOptions(state, ['london', 'Riga']); assert.equal(state.options.length, 2);
  removeOption(state, state.options[0].id);
  addMentionedOptions(state, ['London']); assert.equal(state.options.length, 1);
  addMentionedOptions(state, ['A', 'B', 'C', 'D', 'E', 'F']); assert.equal(state.options.length, 6);
});
...
test('renaming an auto-added option does not re-add its old name from the description', () => {
  ...
  addMentionedOptions(state, ['London', 'Riga']);
  assert.deepEqual(state.options.map(option => option.label), ['Berlin', 'Riga']);
});
