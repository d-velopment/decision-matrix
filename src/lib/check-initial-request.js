 ...
 *
 * Imported by: src/App.svelte and tests/check-initial-request.test.js.
 *
...
export function checkInitialRequest(input) {
  ...
  const english = text
    ...
    .trim();
  if (
    /^(?:help(?: me)?|hi|hello|i (?:don't|do not) know(?: what to do)?|i(?:'m| am) (?:stuck|unsure)|i (?:can't|cannot) decide|what should i do)$/.test(
      english,
    )
  ) {
