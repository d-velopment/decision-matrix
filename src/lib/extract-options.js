/**
 * Conservative offline extraction of alternatives explicitly written by a user.
 * It handles lists, quoted choices, and simple “between” phrasing without semantic inference.
 *
 * Imported by: src/App.svelte, server/ai.js, and radial/core tests.
 */
...
/** Remove list punctuation and surrounding noise from a candidate label. */
const clean = (value) =>
  value
    .trim()
    .replace(/^[\s:;\-–—•\d.)]+|[\s.!?;]+$/g, '')
...
/** Normalize an extracted option for duplicate detection. */
const optionKey = (value) => value.toLocaleLowerCase().replace(/\s+/g, ' ').trim();

/** Extract only explicit, locally recognizable alternatives from a description. */
export function extractOptions(description) {
  const text = description.trim();
  const found = [];

  /** Add a bounded, deduplicated candidate group to the result. */
  const push = (candidates) => {
    const labels = candidates.map(clean).filter(Boolean);
    if (
      labels.length < 2 ||
      labels.length > 6 ||
      labels.some((label) => label.length > 100 || label.split(/\s+/).length > 12)
    )
      return;
  ...
  const either = text.match(
    /(?:\bwhether\s+to\s+|\b(?:choose|choosing|considering)\s+|(?:выбираю|выбрать|рассматриваю|думаю\s+о)\s+)([^.!?\n]+)/iu,
  );
