/**
 * Radial-control geometry and state synchronization helpers.
 * The module converts joystick positions and option rotations into persistent ratings.
 *
 * Imported by: src/App.svelte, src/components/Joystick.svelte, and radial tests.
 */

import { angle, clampPoint as clampPointUtil, TAU } from './utils/math.js';

export { TAU };
export const clampPoint = clampPointUtil;

/** Create a centered radial control with evenly spaced option angles. */
...
/** Convert a control position into one raw rating per option. */
...
/** Synchronize control angles with the current option set. */
export function syncControl(control, options) {
  ...
  for (const option of options) {
    ...
    const existing = Object.values(control.angles)
      .map(angle)
      .sort((a, b) => a - b);
    if (!existing.length) {
      control.angles[option.id] = -Math.PI / 2;
      continue;
    }
    let largest = -1,
      next = 0;
...
/** Rotate one option around a pole and recalculate the control ratings. */
...
/** Validate persisted radial controls against the current option identifiers. */
export function validControls(controls, options) {
  return (
    controls === undefined ||
    ['negative', 'positive'].every((pole) => {
      const c = controls[pole];
      return (
        c &&
        Number.isFinite(c.x) &&
        Number.isFinite(c.y) &&
        Math.hypot(c.x, c.y) <= 1.000001 &&
        typeof c.arranged === 'boolean' &&
