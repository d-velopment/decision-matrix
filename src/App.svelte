<script>
  /**
   * Root decision-matrix application component.
   * It owns the persisted decision state, AI conversation, preparation pages,
   * pair evaluation flow, and Results transition. It is mounted by src/main.js.
   *
   * Imported by: src/main.js.
   * Imports: Icon, Slider, Joystick, and Results from ./components.
   */
  ...
  import { key, pairKey } from './lib/utils/text.js';
  ...
  /** Wait for a short interval used by the typing animations. */
  const wait = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds));

  /** Type a heading with human-like delays and occasional corrected typos. */
  ...
  /** Type all visible Results text nodes concurrently after clearing them. */
  ...
  onMount(() => {
    ...
    /** Convert pointer coordinates into the portrait's bounded gaze offset. */
    ...
    void typeCopy(eyebrowCopy, (value) => {
      typedEyebrow = value;
    });
    void typeCopy(headingCopy, (value) => {
      typedHeading = value;
    });
    void typeCopy(headingEmCopy, (value) => {
      typedHeadingEm = value;
    });
    return () => {
      controller?.abort();
  ...
  /** Mark an input mutation for persistence and result invalidation. */
  function changed() {
    touch(state);
  }

  /** Return the CSS transform for the portrait feature at the current importance. */
  ...
  /** Extract explicit options from the description and add them to state. */
  function extractMentioned() {
    clearTimeout(extractionTimer);
    addMentionedOptions(state, extractOptions(state.description));
  }

  /** Update the description and schedule local option extraction. */
  ...
  /** Finish pending option extraction after the description loses focus. */
  ...
  /** Navigate between the major decision steps and reset the scroll position. */
  function setStep(step) {
    if (step !== 'prepare' && !ready) return;
    if (step === 'results') state.lastCalculatedRevision = state.inputRevision;
    if (step === 'results') void typeResultsText();
    else resultsTypeRun++;
    state.currentStep = step;
    window.scrollTo({ top: 0, behavior: 'instant' });
  }

  /** Navigate between the preparation subpages. */
  ...
  /** Select an adjacent pair and scroll back to its evaluation header. */
  ...
  /** Scroll to the conversation or the newest assistant response after rendering. */
  ...
  /** Add a blank option and focus its new input. */
  async function addManualOption() {
    const option = addOption(state);
    if (option) {
      await tick();
      document.getElementById(`option-${option.id}`)?.focus();
    }
  }

  /** Add a blank pair and focus its negative-label input. */
  async function addManualPair() {
    const pair = addPair(state);
    if (pair) {
      await tick();
      document.getElementById(`negative-${pair.id}`)?.focus();
    }
  }

  /** Accept one pending option suggestion into the editable list. */
  ...
  /** Accept one pending pair suggestion into the editable list. */
  function acceptPair(pair) {
    if (state.pairs.length >= 30) return;
    if (!state.pairs.some((item) => pairKey(item) === pairKey(pair)))
      addPair(state, pair.negativeLabel, pair.positiveLabel);
    state.suggestions.pairs = state.suggestions.pairs.filter((item) => pairKey(item) !== pairKey(pair));
  }

  /** Accept every currently visible option suggestion. */
  function acceptAllOptions() {
    for (const option of [...state.suggestions.options]) acceptOption(option);
  }

  /** Accept every currently visible pair suggestion. */
  function acceptAllPairs() {
    for (const pair of [...state.suggestions.pairs]) acceptPair(pair);
  }

  /** Dismiss a suggestion and remember it for future provider requests. */
  function dismiss(type, item) {
    const itemKey = type === 'options' ? key(item) : pairKey(item);
    state.dismissed[type].push(itemKey);
    state.suggestions[type] = state.suggestions[type].filter(
      (value) => (type === 'options' ? key(value) : pairKey(value)) !== itemKey,
    );
  }

  /** Request, validate, and merge AI suggestions for the current conversation. */
  async function suggest(intent = 'initial') {
    ...
    if (intent === 'reply') {
      state.messages.push({ role: 'user', content: reply.trim() });
      reply = '';
    }
    extractMentioned();
    ...
    try {
      const response = await fetch('/api/suggest', {
        ...
        body: JSON.stringify({
          ...
          suggestedPairs: state.suggestions.pairs.map(({ negativeLabel, positiveLabel }) => ({
            negativeLabel,
            positiveLabel,
          })),
      ...
      if (state.description === descriptionAtRequest && Array.isArray(data.mentionedOptions)) {
        const source = [
          ...
          .join('\n')
          .toLocaleLowerCase();
        const grounded = data.mentionedOptions.filter(
          (item) =>
            item &&
      ...
      /* Intentionally quiet: manual editing is always available. */
    } finally {
      clearTimeout(timeout);
      if (number === requestNumber) busy = false;
    }
  }

  /** Confirmed reset of the current decision and all local conversation state. */
  function reset() {
    initialRequestHint = '';
    clearTimeout(extractionTimer);
    controller?.abort();
...
<svelte:head
  ><title>Decision Matrix — {state.currentStep === 'results' ? 'Your way forward' : 'A little clarity'}</title
  ></svelte:head
>

{#snippet navigation()}
  <nav class="step-nav" aria-label="Decision steps">
    {#each [['home', 'Home'], ['options', 'Options'], ['criteria', 'Personal criteria'], ['evaluate', 'Weigh it up'], ['results', 'Results']] as [step, label], index (step)}
      {@const active =
        step === 'home'
          ? state.currentStep === 'prepare' && state.preparePage === 'mind'
          : step === 'evaluate' || step === 'results'
            ? state.currentStep === step
            : state.currentStep === 'prepare' && state.preparePage === step}
      <button
        class:active
        aria-current={active ? 'step' : undefined}
        disabled={(step === 'evaluate' || step === 'results') && !ready}
        onclick={() =>
          step === 'home' || step === 'options' || step === 'criteria'
            ? setPreparePage(step === 'home' ? 'mind' : step)
            : setStep(step)}><span class="step-number">{step === 'home' ? '⌂' : `0${index}`}</span>{label}</button
      >
...
<div
  class="app-shell"
  class:welcome={state.currentStep === 'prepare' && state.preparePage === 'mind'}
  class:prepare-inner={state.currentStep === 'prepare' && state.preparePage !== 'mind'}
>
  <header class="site-header">
    <a class="brand" href="/" aria-label="Decision Matrix home"
      ><img src="/favicon.svg" alt="" width="36" height="36" /><span
        >decision<span class="brand-light">matrix</span></span
      ></a
    >
    <div class="header-right">
      <span class="save-state"
        ><span class="tiny-dot"></span>{saveAvailable ? 'Saved on this device' : 'This session only'}</span
      ><button class="text-button reset" onclick={() => dialog.showModal()}
        ><Icon name="reset" size={16} /><span>Start over</span></button
      >
    </div>
  </header>
  ...
  <main>
    {#if state.currentStep === 'prepare'}
      <div class="page-heading">
        <div>
          <div class="eyebrow"><span class="tiny-dot"></span> {typedEyebrow}</div>
          <h1>{typedHeading}<br /><em>{typedHeadingEm}</em></h1>
        </div>
        <div class="page-heading-copy">
          {#if state.preparePage !== 'mind'}<button class="text-button" onclick={() => setPreparePage('mind')}
              ><Icon name="back" size={16} />Back to What’s on your mind</button
            >{/if}
          <p>
            Start with AI in <em>What’s on your mind?</em><br class="desktop" /> Then lay out your options and make room for
            what matters.
          </p>
        </div>
      </div>
      ...
      <div class="prepare-layout prepare-{state.preparePage}">
        ...
        <aside class:situation-hidden={state.preparePage !== 'mind'} class="situation-panel">
          ...
          <p class="welcome-reassurance" aria-live="polite">
            {#each welcomeReassurances as reassurance, index (reassurance)}<span style={`--reassurance-index:${index}`}
                >{reassurance}</span
              >{/each}
          </p>
          <p class="muted">
            Describe the area of life, the decision you’re facing, and the options you’re considering.
          </p>
          ...
          <textarea
            ...
            onblur={descriptionBlurred}></textarea>
          ...
          <button
            ...
            disabled={busy || !state.description.trim()}
            onclick={() => suggest('initial')}
            >{busy ? 'Thinking…' : 'Let’s begin'}{#if !busy}<Icon name="arrow" size={18} />{/if}</button
          >
          {#if provider === 'demo'}<p class="demo-note">
              <span class="demo-dot"></span> Demo mode · sample suggestions, no API calls
            </p>{:else}<p class="demo-note">Your description is shared with OpenAI for suggestions.</p>{/if}
          {#if state.messages.length}
            <div class="conversation" aria-label="AI conversation" aria-live="polite">
              {#each state.messages as message, index (index)}<div class="message {message.role}">
                  <span
            ...
            {#if !aiContextReady}<form
              ...
              >
                <label class="sr-only" for="reply">Reply to assistant</label><textarea
                  id="reply"
                  rows="2"
                  maxlength="6000"
                  placeholder="Add a little more context…"
                  bind:value={reply}></textarea><button
                  class="button secondary full"
                  disabled={busy || !reply.trim()}
                  type="submit">Send reply <Icon name="arrow" size={16} /></button
                >
              </form>{/if}
          {/if}
          {#if state.preparePage === 'mind' && aiContextReady && (state.options.length || state.suggestions.options.length)}
            <div class="options-ready">
              <p>Your options and pairs are ready to review.</p>
              <button class="button primary full" onclick={() => setPreparePage('options')}
                >Fill in Options <Icon name="arrow" size={18} /></button
              >
            </div>
          {/if}
          <div class="quiet-note">
            <Icon name="shield" size={20} />
            <p>Your ratings and results stay in your browser. This space is yours to change.</p>
          </div>
        </aside>

        <div class="workspace">
          {#if suggestionCount}<div class="sr-only" role="region" aria-label="Suggestions">
              {state.suggestions.options.join(' ')}
              {state.suggestions.pairs.map((pair) => `${pair.negativeLabel} ${pair.positiveLabel}`).join(' ')}
            </div>{/if}
          <section
            class:hidden-page={state.preparePage !== 'options'}
            class="options-section"
            aria-label="Your options"
          >
            <div class="section-header">
              <div>
                <div class="section-label"><span class="section-number">02</span> THE POSSIBILITIES</div>
                <h2>Your options <span class="count">{state.options.length} / 6</span></h2>
              </div>
            </div>
            <div class="option-list">
              {#each state.options as option, index (option.id)}
                <div class="option-row">
                  <span class="option-letter">{String.fromCharCode(65 + index)}</span><label
                    class="sr-only"
                    for={`option-${option.id}`}>Option {index + 1}</label
                  ><input
                    id={`option-${option.id}`}
                    maxlength="160"
                    dir="auto"
                    placeholder="Name this possibility…"
                    bind:value={option.label}
                    oninput={changed}
                  /><button
                    class="icon-button"
                    aria-label={`Remove option ${index + 1}`}
                    onclick={() => removeOption(state, option.id)}><Icon name="close" size={16} /></button
                  >
                </div>
              {:else}<div class="empty-state">
                  <span class="empty-symbol">↗</span>
                  <p>A few possibilities.<br /><span>Add your own, or explore with AI.</span></p>
                </div>{/each}
            </div>
            {#if state.suggestions.options.length}
              <div class="suggestions suggestions-inline" aria-label="Suggestions">
                <div class="section-header">
                  <div>
                    <div class="eyebrow"><Icon name="spark" size={14} /> KEEP WHAT FEELS RIGHT</div>
                    <h3>Options</h3>
                  </div>
                  <button
                    class="text-button"
                    aria-label="Add all options"
                    onclick={acceptAllOptions}
                    disabled={state.options.length >= 6}
                    ><span aria-hidden="true">Add all</span><Icon name="plus" size={16} /></button
                  >
                </div>
                {#each state.suggestions.options as option (option)}<div class="suggestion-row">
                    <span dir="auto">{option}</span><button
                      class="small-button"
                      disabled={state.options.length >= 6}
                      onclick={() => acceptOption(option)}>Add</button
                    ><button
                      class="icon-button"
                      aria-label={`Dismiss ${option}`}
                      onclick={() => dismiss('options', option)}><Icon name="close" size={16} /></button
                    >
                  </div>{/each}
              </div>
            {/if}
            <div class="section-actions">
              {#if state.preparePage === 'options' && suggestMoreAvailable.options}<button
                  class="text-button more-button"
                  disabled={busy ||
                    !state.description.trim() ||
                    (state.options.length >= 6 && state.pairs.length >= 30)}
                  onclick={() => suggest('more')}
                  ><Icon name="spark" size={16} />{busy ? 'Thinking…' : 'Suggest more'}</button
                >{/if}<button
                class="text-button section-add"
                onclick={addManualOption}
                disabled={state.options.length >= 6}><Icon name="plus" size={16} />Add option</button
              >
            </div>
            <div class="page-next">
              <p>When your options feel complete, move on to what matters.</p>
              <button
                class="button primary"
                disabled={state.options.filter((option) => option.label.trim()).length < 2}
                onclick={() => setPreparePage('criteria')}>Personal criteria <Icon name="arrow" /></button
              >
            </div>
          </section>
          ...
          <section class:hidden-page={state.preparePage !== 'criteria'} aria-label="What matters to you">
            <div class="section-header">
              <div>
                <div class="section-label"><span class="section-number">03</span> WHAT MATTERS</div>
                <h2>Make it personal <span class="count">{state.pairs.length} / 30</span></h2>
              </div>
            </div>
              ...
              {#each state.pairs as pair, index (pair.id)}
                <article class="pair-card">
                  <div class="pair-card-top">
                    <span class="pair-index">PAIR {String(index + 1).padStart(2, '0')}</span><button
                      class="icon-button"
                      aria-label={`Remove pair ${index + 1}`}
                      onclick={() => removePair(state, pair.id)}><Icon name="close" size={16} /></button
                    >
                  </div>
                  <div class="pair-labels">
                    <label
                      ><span class="pole-label"><span class="pole-dot negative"></span>Negative</span><input
                        id={`negative-${pair.id}`}
                        aria-label={`Negative state ${index + 1}`}
                        maxlength="160"
                        dir="auto"
                        placeholder="What feels difficult?"
                        bind:value={pair.negativeLabel}
                        oninput={changed}
                      /></label
                    ><span class="pair-divider">↔</span><label
                      ><span class="pole-label"><span class="pole-dot positive"></span>Positive</span><input
                        aria-label={`Positive state ${index + 1}`}
                        maxlength="160"
                        dir="auto"
                        placeholder="Its positive alternative…"
                        bind:value={pair.positiveLabel}
                        oninput={changed}
                      /></label
                    >
                  </div>
                </article>
              {:else}<div class="empty-state pair-empty">
                  <span class="empty-symbol">↔</span>
                  <p>What pulls you in? What holds you back?<br /><span>Your first pair starts here.</span></p>
                </div>{/each}
            </div>
            {#if state.suggestions.pairs.length}
              <div class="suggestions suggestions-inline" aria-label="Pair suggestions">
                <div class="section-header">
                  ...
                  <button
                    class="text-button"
                    aria-label="Add all pairs"
                    onclick={acceptAllPairs}
                    disabled={state.pairs.length >= 30}
                    ><span aria-hidden="true">Add all</span><Icon name="plus" size={16} /></button
                  >
                </div>
                {#each state.suggestions.pairs as pair (`${pair.negativeLabel}|${pair.positiveLabel}`)}<div
                    class="suggestion-row"
                  >
                    <span class="suggested-pair" dir="auto"
                      >{pair.negativeLabel}<span class="muted"> ↔ </span>{pair.positiveLabel}</span
                    ><button class="small-button" disabled={state.pairs.length >= 30} onclick={() => acceptPair(pair)}
                      >Add</button
                    ><button
                      class="icon-button"
                      aria-label={`Dismiss ${pair.negativeLabel}`}
                      onclick={() => dismiss('pairs', pair)}><Icon name="close" size={16} /></button
                    >
                  </div>{/each}
              </div>
            {/if}
            <div class="section-actions">
              {#if state.preparePage === 'criteria' && suggestMoreAvailable.pairs}<button
                  class="text-button more-button"
                  disabled={busy ||
                    !state.description.trim() ||
                    (state.options.length >= 6 && state.pairs.length >= 30)}
                ...
                disabled={state.pairs.length >= 30}><Icon name="plus" size={16} />Add pair</button
              >
            </div>
            <div class="page-next">
              <p>Next, we’ll compare how each experience feels for every option.</p>
              <button
                class="button primary"
                aria-label="Weigh my options"
                disabled={!ready}
                onclick={() => setStep('evaluate')}>Weigh it up <Icon name="arrow" /></button
              >
            </div>
          </section>
        </div>
    ...
    {:else if state.currentStep === 'evaluate' && currentPair}
      <div class="evaluation-heading">
        <div>
          <div class="eyebrow"><span class="tiny-dot"></span> ONE EXPERIENCE AT A TIME</div>
      ...
      <div class="evaluation-layout">
        <aside class="pair-navigation">
          <div class="section-label">YOUR PAIRS <span class="count">{state.pairs.length}</span></div>
          {#each state.pairs as pair, index (pair.id)}<button
              class:chosen={pair.id === state.currentPairId}
              aria-current={pair.id === state.currentPairId ? 'step' : undefined}
              onclick={() => {
                state.currentPairId = pair.id;
              }}
              ><span>{String(index + 1).padStart(2, '0')}</span><span dir="auto">{pair.positiveLabel}</span
              >{#if pair.id === state.currentPairId}<Icon name="arrow" size={15} />{/if}</button
            >{/each}
        </aside>
        <section class="evaluation-card">
          <div class="evaluation-title">
            <div class="eyebrow">
              PAIR {String(pairIndex + 1).padStart(2, '0')} OF {String(state.pairs.length).padStart(2, '0')}
            </div>
            <h2 dir="auto">{currentPair.negativeLabel}<span>&nbsp;&amp;&nbsp;</span>{currentPair.positiveLabel}</h2>
          </div>
          <div class="importance-block">
            <h3>How much does this matter to you?</h3>
            <Slider
              importance
              label="Importance of this pair"
              value={currentPair.importanceRaw}
              onchange={(value) => {
                currentPair.importanceRaw = value;
                changed();
              }}
            />
          </div>
          <div class="association-columns">
            {#each ['negative', 'positive'] as pole (pole)}
              <section class="association-column {pole}">
                <span class="pole-label"
                  ><span class="pole-dot {pole}"></span>{pole === 'negative'
                    ? 'NEGATIVE EXPERIENCE'
                    : 'POSITIVE ALTERNATIVE'}</span
                >
                <h3 dir="auto">{currentPair[`${pole}Label`]}</h3>
                {#key currentPair.id}
                  <Joystick
                    options={state.options}
                    control={currentPair.controls[pole]}
                    label={currentPair[`${pole}Label`]}
                    tone={pole}
                    onmove={(x, y) => {
                      moveControl(currentPair, state.options, pole, x, y);
                      changed();
                    }}
                    onrotate={(optionId, radians) => {
                      moveOption(currentPair, state.options, pole, optionId, radians);
                      changed();
                    }}
                  />
                {/key}
          ...
          <div class="evaluation-footer">
            <button class="text-button" disabled={pairIndex === 0} onclick={() => navigatePair(-1)}
              ><Icon name="back" size={18} />Previous</button
            ><span class="pair-position">{pairIndex + 1} / {state.pairs.length}</span
            >{#if pairIndex < state.pairs.length - 1}<button class="button secondary" onclick={() => navigatePair(1)}
                >Next pair <Icon name="arrow" size={18} /></button
              >{:else}<button class="button primary" onclick={() => setStep('results')}
                >Calculate <Icon name="arrow" size={18} /></button
              >{/if}
          </div>
        </section>
      </div>
      {#if pairIndex < state.pairs.length - 1}<div class="calculate-anytime">
          <button class="text-button" onclick={() => setStep('results')}
            >Calculate now <Icon name="arrow" size={16} /></button
          >
        </div>{/if}
  ...
  <footer class="site-footer">
    <span>Less noise. More perspective.</span><span>Made for your kind of decision.</span>
  </footer>
</div>

<dialog bind:this={dialog} class="reset-dialog">
  <form method="dialog">
    <div class="eyebrow">A FRESH PAGE</div>
    <h2>Start over?</h2>
    <p>This will clear your current decision, options, and ratings from this device.</p>
    <div class="dialog-actions">
      <button class="button secondary" value="cancel">Keep my decision</button><button
        class="button primary"
        type="button"
